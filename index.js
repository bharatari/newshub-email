...
function sendEmail(to, subject, body, template, attachments) {
  return new Promise((resolve, reject) => {
    ...
    if (attachments) {
      for (let i = 0; i < attachments.length; i++) {
        const attachment = new helpers.Attachment(attachments[i]);
  ...
  if (message) {
    if (message.environment === "production") {
      async.each(message.users, (user, callback) => {
        sendEmail(user.email, message.subject, message.body, message.template, message.attachments)
          .then(() => {
            callback();
          })
          .catch((e) => {
